...
Output MUST be valid JSON matching this TypeScript interface:
{
  ...
  statement: string; (The English description of the problem. PLAIN TEXT ONLY. Use normal symbols like $ and %. Do NOT use LaTeX commands or delimiters. Write "$250" not "\\$250". Write "20%" not "20\\%". No backslashes at all.)
...
Make it engaging and educational. The problem should be specifically about ${selection.subtopic}, not a generic problem.`;
    ...
    try {
        ...
        // Clean the statement of ALL LaTeX artifacts the AI sometimes adds
        const cleanStatement = (problem.statement || problem.title)
            // \$ → $  (LaTeX-escaped dollar sign → real dollar sign)
            .replace(/\\\$/g, '$')
            // \% → %  (LaTeX-escaped percent → real percent)
            .replace(/\\%/g, '%')
            // Remove \( and \) delimiters
            .replace(/\\\(/g, '')
            .replace(/\\\)/g, '')
            // Remove $...$ inline math wrappers but keep content
            .replace(/\$([^$]+)\$/g, '$1')
            // Remove \text{...} wrappers, keep content
            .replace(/\\text\{([^}]*)\}/g, '$1')
            // Remove remaining LaTeX commands (\frac, \sqrt etc.)
            .replace(/\\[a-zA-Z]+/g, '')
